...
// Prototype 07.1: 交換で非成立から成立へ変化したラインkeyの履歴(追加のみ、Reset以外では削除しない)。
// 「直前の交換」だけでなく、それ以前の交換で成立させたラインの＝表示も維持するために使う。
// 現在の状態そのもの(キャッシュ)ではなく「プレイ中に成立させた」という履歴。
// 崩れても記録からは消さない(表示は現在状態とのANDで決まるため自然に追従する)。
// Undoでは変更しない(崩した交換をUndoして再成立すれば、履歴が残っているため＝が復活する)。
// Resetでのみ空にする。
let solvedViaSwapLineKeys = new Set();
// Prototype 07.1: 自動解読済みsealed-fixedセルの値(cellDomKey -> value)。
// correctValueやCUBE_DATAの封印値を直接参照せず、公開済みの値だけから算出する
// (measure.jsのautoDecodeSealedCellsへ渡すknownValuesの構築時にも同じ制約を守る)。
// Undoでは消さない。Resetでのみ空にする。ページ再読込を越える保存は行わない。
let decodedSealedCells = new Map();

// 109ラインの座標{z,y,x}をrepair-mainのcellDomKey(`${L}-${r}-${c}`)へ変換する。
function lineCellKey(cell){ return cellDomKey(cell.z+1, cell.y, cell.x); }

// 自動解読の入力となるknownValuesを構築する: movableの現在値 + revealed-fixedの値 + 既に解読済みのsealed値。
// revealed-fixedは常に画面表示されている値なので、CUBE_DATAから読んでも新たな漏洩にはならない
// (=既存のrepairGridValueが返す値そのもの)。未解読のsealed-fixedはここに一切含めない。
function buildKnownValuesForDecode(){
  const known = new Map();
  for(const cell of REPAIR_CELLS){
    const key = cellDomKey(cell.L, cell.r, cell.c);
    known.set(key, repairGridValue(repairState, cell.L, cell.r, cell.c));
  }
  for(const cell of REVEALED_FIXED_CELLS){
    const key = cellDomKey(cell.L, cell.r, cell.c);
    known.set(key, repairGridValue(repairState, cell.L, cell.r, cell.c));
  }
  for(const [key, value] of decodedSealedCells) known.set(key, value);
  return known;
}

// 成功した交換の完了後、描画前に固定点まで自動解読する。
// autoDecodeSealedCells自体はrepairState/CUBE_DATA/correctValueへ一切アクセスしない純粋関数。
function runAutoDecode(){
  const lineStatuses = computeAllLineStatuses();
  const known = buildKnownValuesForDecode();
  const { decoded, contributingLineKeys } = autoDecodeSealedCells(ALL_LINES, lineStatuses, known, lineCellKey);
  for(const [key, value] of decoded) decodedSealedCells.set(key, value);
  for(const lineKey of contributingLineKeys) solvedViaSwapLineKeys.add(lineKey);
}
...
function triggerSwap(a, b){
  ...
  return animateSwap(a, b, valueA, valueB).then(()=>{
    ...
    // 成立履歴(solvedViaSwapLineKeys)は「プレイ中に非成立→成立させたライン」の記録であり、
    // 現在の状態のキャッシュではない。崩れても削除しない(Resetでのみ全消去)。
    // 交換前後とも成立(shared lineが崩れずそのまま等)だった場合はchange==='unchanged'となり、
    // 「触れただけ」では追加しない。change==='solved'(非成立→成立)のときだけ追加する。
    for(const { line, change } of feedback){
      if(change === 'solved') solvedViaSwapLineKeys.add(line.key);
    }
    highlightedLineKey = null; // 交換確定で古いラインフォーカスを解除する
    animating = false;

    runAutoDecode(); // 描画前に固定点まで自動解読する(交換で崩れた側の再解読は起きない設計)
...
// sealed-fixedセルへ小さな単色鍵穴(円+くさび形)を追加する。数字や内部値は一切持たせない。
// decoded=false: 従来どおりラベル位置中央に通常サイズで表示する。
// decoded=true : 中央には解読済みの数字を表示するため、同じ形状を縮小し隅(右上)へ寄せる。
// 既に同じdecoded状態のものが存在する場合は何もしない(renderAll/Reset/交換で連続呼び出しされても増殖しない)。
// decoded状態が変化した場合だけ、既存の鍵穴を作り直す(形状の定義自体は流用し、別アイコンは追加しない)。
function ensureSealedKeyhole(g, labelEl, decoded){
  const decodedFlag = decoded ? 'true' : 'false';
  const existing = g.querySelector('.cell-keyhole');
  if(existing && existing.dataset.decoded === decodedFlag) return;
  if(existing) existing.remove();
  if(!labelEl) return;
  const cx0 = parseFloat(labelEl.getAttribute('x'));
  const cy0 = parseFloat(labelEl.getAttribute('y'));
  if(Number.isNaN(cx0) || Number.isNaN(cy0)) return;

  // decoded時は同じ相対形状を0.5倍に縮小し、ラベル位置から右上へオフセットして隅へ寄せる。
  const scale = decoded ? 0.5 : 1;
  const cx = decoded ? cx0 + 9 : cx0;
  const cy = decoded ? cy0 - 8 : cy0;

  const kh = document.createElementNS('http://www.w3.org/2000/svg','g');
  kh.setAttribute('class','cell-keyhole');
  kh.dataset.decoded = decodedFlag;
  kh.setAttribute('pointer-events','none');
  kh.setAttribute('aria-hidden','true');

  const hole = document.createElementNS('http://www.w3.org/2000/svg','circle');
  const holeR = 2.6 * scale, holeCy = cy - 2.4 * scale;
  hole.setAttribute('cx', cx);
  hole.setAttribute('cy', holeCy);
  hole.setAttribute('r', holeR);
  kh.appendChild(hole);

  const stemTopY = holeCy + holeR * 0.55, stemBottomY = cy + 6.5 * scale;
  const stemTopHalfW = 1.5 * scale, stemBottomHalfW = 3.2 * scale;
...
function resetPuzzle(){
  ...
  solvedViaSwapLineKeys = new Set(); // Reset時は継続＝表示の記録も消す
  decodedSealedCells = new Map(); // Reset時は自動解読済みの数字も消す
  ...
  for(let L=1; L<=LEVELS; L++){
    for(let r=0;r<N;r++){
      for(let c=0;c<N;c++){
        ...
        const decodedValue = state === 'sealed-fixed' ? decodedSealedCells.get(key) : undefined;
        const label = g.querySelector('.cube-label');
        if(label){
          if(state === 'sealed-fixed') label.textContent = decodedValue !== undefined ? decodedValue : '';
          else label.textContent = value;
        }

        if(state === 'sealed-fixed'){
          ensureSealedKeyhole(g, label, decodedValue !== undefined);
        } else {
        ...
        g.classList.toggle('decoded-sealed', decodedValue !== undefined);
...
function applyFlatLabel(el, hit, line, lineStatuses, indicator, eqSymbol, group){
  ...
  // 直前交換の結果(change)だけでなく、それ以前の交換で成立させ、今も成立中のラインも
  // ＝を継続表示する対象に含める(初期から成立済みのライン=記録されていないラインは対象外のまま)。
  const persistentlyEqual = status === '=' && solvedViaSwapLineKeys.has(line.key);
  el.classList.remove('stat-over','stat-under');

  if(status === '=' && change === undefined && !persistentlyEqual){
    setDirectText(el, '');
