...
console.log('== production_match_tests: 保存候補とproduction REPAIR_CELLSの完全一致 ==');
{
  const ctx = {};
  ...
  const candidateByKey = new Map(savedArtifact.cells.map(cell => [
  ...
  check('candidate cellsとproduction REPAIR_CELLSの座標集合が完全一致', coordSetsMatch);
  ...
  check('candidateの初期表示値とproductionのinitialValueが各座標で完全一致', initialValueMismatchCount === 0);
